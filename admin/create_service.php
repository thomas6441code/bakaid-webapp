...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $beneficalGroup = htmlspecialchars(trim($_POST['beneficalGroup']), ENT_QUOTES, 'UTF-8');
    $locations = htmlspecialchars(trim($_POST['locations']), ENT_QUOTES, 'UTF-8');
    $category = htmlspecialchars(trim($_POST['category']), ENT_QUOTES, 'UTF-8');
    $fundSource = htmlspecialchars(trim($_POST['fundSource']), ENT_QUOTES, 'UTF-8');
    ...
    $stmt = $mysqli->prepare("INSERT INTO service (title, description, beneficalGroup, locations, category, fundSource, status, image, image1, image2, image3, image4, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())");
    $stmt->bind_param("ssssssssssss", $title, $description, $beneficalGroup, $locations, $category, $fundSource, $status, $uploadedMainImage, $uploadedImage1, $uploadedImage2, $uploadedImage3, $uploadedImage4);
...
<div class="mt-16 md:ml-44 max-w-3xl py-4 pb-10">
    <div class="min-w-full px-3 text-lg md:px-5">
        ...
        <form id="projectForm" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                <input type="text" name="beneficalGroup" id="beneficalGroup" placeholder="Beneficial Group" required
                    class="block w-full border p-3 rounded mb-2" />
            </div>
            ...
            <div class="md:col-span-2">
                <input type="text" name="fundSource" id="fundSource" placeholder="Fund Source" required
                    class="block w-full border p-3 rounded mb-2" />
            </div>
