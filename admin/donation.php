...
$sql = "SELECT * FROM donation LIMIT $startIndex, $donationPerPage";
$result = $mysqli->query($sql);

$totalDonationsSql = "SELECT COUNT(*) as total FROM donation";
        ...
        <div id="donationsTable" class="overflow-x-auto scrollbar-hidden">
            <table class="min-w-full rounded-lg shadow-md">
                ...
                <tbody id="donationRows">
                    <?php if ($result->num_rows > 0): ?>
                        <?php while($row = $result->fetch_assoc()): ?>
                            <tr class="border-b border-gray-300">
                                ...
                                <td class="border px-4 py-2"><?php echo $row['fullName'] . ' ' . $row['lastName']; ?></td>
                                <td class="border px-4 py-2"><?php echo $row['email']; ?></td>
                                <td class="border px-4 py-2"><?php echo number_format($row['amount'], 2); ?></td>
                                <td class="border px-4 py-2"><?php echo date('Y-m-d H:i:s', strtotime($row['createdAt'])); ?></td>
                                <td class="border px-4 py-2"><?php echo $row['isRead'] ? 'Received' : 'Not Received'; ?></td>
                                <td class="border px-4 py-2">
                                    <div class="flex space-x-2">
                                        <?php if ($row['isRead']): ?>
                                            <button onclick="updateDonationStatus(<?php echo $row['id']; ?>, false)" class="bg-yellow-500 text-white px-4 py-2 rounded">
                                                Unmark
                                            </button>
                                        <?php else: ?>
                                            <button onclick="updateDonationStatus(<?php echo $row['id']; ?>, true)" class="bg-green-500 text-white px-4 py-2 rounded">
                                                Mark
                                            </button>
                                        <?php endif; ?>
                                    </div>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="6" class="border px-4 py-2 text-center">No donations found.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
<script>
    const updateDonationStatus = async (id, isRead) => {
        try {
            const response = await fetch(`donations.php`, {
                method: 'POST',
