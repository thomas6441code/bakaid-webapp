...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $mainImagePath = $slide['mainImage'] ?? '';
    $image1Path = $slide['image1'] ?? '';
    $image2Path = $slide['image2'] ?? '';
    $image3Path = $slide['image3'] ?? '';
    $image4Path = $slide['image4'] ?? '';
    ...
    $stmt = $mysqli->prepare("UPDATE project SET title = ?, description = ?, moreDescription = ?, date = ?, location = ?, mainImage = ?, image1 = ?, image2 = ?, image3 = ?, image4 = ?, updatedAt = NOW() WHERE id = ?");
    $stmt->bind_param("ssssssssssi", $title, $description, $moreDescription, $date, $location, $mainImagePath, $image1Path, $image2Path, $image3Path, $image4Path, $slideId);
            ...
            <form id="projectForm" class="space-y-6" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <textarea name="moreDescription" id="moreDescription" placeholder="Additional details" required
                        rows="5" class="w-full p-3 border rounded-md resize-none"><?php echo htmlspecialchars($slide['moreDescription'] ?? ''); ?></textarea>
                </div>
                ...
                <?php if ($slide['mainImage']): ?>
                    <div class="mb-6">
                        <label class="block text-lg font-semibold mb-2">Current Main Image</label>
                        <img src="../public/images/<?php echo $slide['mainImage']; ?>" alt="Current Main Image" class='object-cover h-56 w-full rounded-md cursor-pointer' />
                    </div>
                <?php endif; ?>
                <?php if ($slide['image1']): ?>
                    <div class="mb-6">
                        <label class="block text-lg font-semibold mb-2">Current Image 1</label>
                        <img src="../public/images/<?php echo $slide['image1']; ?>" alt="Current Image 1" class='object-cover h-56 w-full rounded-md cursor-pointer' />
                    </div>
                <?php endif; ?>
                <?php if ($slide['image2']): ?>
                    <div class="mb-6">
                        <label class="block text-lg font-semibold mb-2">Current Image 2</label>
                        <img src="../public/images/<?php echo $slide['image2']; ?>" alt="Current Image 2" class='object-cover h-56 w-full rounded-md cursor-pointer' />
                    </div>
                <?php endif; ?>
                <?php if ($slide['image3']): ?>
                    <div class="mb-6">
                        <label class="block text-lg font-semibold mb-2">Current Image 3</label>
                        <img src="../public/images/<?php echo $slide['image3']; ?>" alt="Current Image 3" class='object-cover h-56 w-full rounded-md cursor-pointer' />
                    </div>
                <?php endif; ?>
                <?php if ($slide['image4']): ?>
                    <div class="mb-6">
                        <label class="block text-lg font-semibold mb-2">Current Image 4</label>
                        <img src="../public/images/<?php echo $slide['image4']; ?>" alt="Current Image 4" class='object-cover h-56 w-full rounded-md cursor-pointer' />
                    </div>
                <?php endif; ?>
                    </div>
